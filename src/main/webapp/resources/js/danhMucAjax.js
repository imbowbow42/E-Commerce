$(document).ready(function(){
	...
	function ajaxGet(page){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8088/api/danh-muc/all" + "?page=" + page,
...
//    });
	...
	function ajaxPost(){
    	...
    	$.ajax({
			...
			url : "http://localhost:8088/api/danh-muc/save",
    ...
    $(document).on("click",".btnCapNhatDanhMuc",function(){
		...
		var href = "http://localhost:8088/api/danh-muc/"+danhMucId;
    ...
    function ajaxPut(){
    	...
    	$.ajax({
			...
			type : "PUT",
			contentType : "application/json",
			url : "http://localhost:8088/api/danh-muc/update",
			data : JSON.stringify(formData),
            // dataType : 'json',
			success : function(response) {
    ...
    $(document).on("click",".btnXoaDanhMuc", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8088/api/danh-muc/delete/" + danhMucId,
